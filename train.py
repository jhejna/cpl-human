...
def main(_):
    ...
    if FLAGS.cpl:
        FLAGS.logging.output_dir = os.path.join(FLAGS.logging.output_dir, "cpl")
    else:
    ...
    comment += f"_cpl{str(FLAGS.cpl)}"
    comment += f"_temp{str(FLAGS.dist_temperature)}"
    comment += f"_bc{str(FLAGS.bc_steps)}"
    ...
    for i in range(FLAGS.max_steps + 1):
        ...
        with Timer() as timer:
            ...
            train_info = prefix_metrics(agent.update(batch, i), 'train')
